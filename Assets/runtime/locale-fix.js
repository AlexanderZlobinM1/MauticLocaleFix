(function (window) {
    ...
    function removeTimezoneLabel(input) {
        ...
        if (badge && typeof badge.remove === 'function') {
            if (hasClass(wrapper, 'mautic-locale-fix-timezone-control')) {
                wrapper = badge.parentElement;
            }
            badge.remove();
        }
        if (wrapper && wrapper.classList && typeof wrapper.classList.remove === 'function') {
            wrapper.classList.remove('mautic-locale-fix-timezone-control--narrow');
        }

        // Unwrap controls generated by the 1.0.46 candidate that introduced
        // an outer layout wrapper, so an in-page cache refresh restores Mautic's
        // original field structure before the compact-only patch is applied.
        var legacyWrapper = input && input.parentElement;
        while (legacyWrapper && !hasClass(legacyWrapper, 'mautic-locale-fix-timezone-control')) {
            legacyWrapper = legacyWrapper.parentElement;
        }
        if (legacyWrapper && legacyWrapper.children && legacyWrapper.children.length === 1 && legacyWrapper.parentNode && typeof legacyWrapper.parentNode.insertBefore === 'function') {
            legacyWrapper.parentNode.insertBefore(legacyWrapper.children[0], legacyWrapper);
            if (typeof legacyWrapper.remove === 'function') {
    ...
    function trackTimezoneLabelGroup(group) {
        ...
        if (typeof group.getBoundingClientRect === 'function') {
            updateTimezoneLabelLayout(group, group.getBoundingClientRect().width);
        }
        if (runtime.timezoneLabelResizeObserver && typeof runtime.timezoneLabelResizeObserver.observe === 'function') {
            runtime.timezoneLabelResizeObserver.observe(group);
        }
    }
    ...
    function deactivateRuntime() {
        ...
        if (runtime.timezoneLabelResizeObserver) {
            runtime.timezoneLabelResizeObserver.disconnect();
            runtime.timezoneLabelResizeObserver = null;
        }
    ...
    function installRuntimeLoop() {
        ...
        if ('ResizeObserver' in window && !runtime.timezoneLabelResizeObserver) {
            runtime.timezoneLabelResizeObserver = new window.ResizeObserver(function (entries) {
                Array.prototype.forEach.call(entries, function (entry) {
                    var width = entry.contentRect && entry.contentRect.width;
                    if (!(width > 0) && entry.target && typeof entry.target.getBoundingClientRect === 'function') {
                        width = entry.target.getBoundingClientRect().width;
                    }
                    updateTimezoneLabelLayout(entry.target, width);
                });
            });
        }
