(function (window) {
    ...
    function removeTimezoneLabel(input) {
        ...
        // Unwrap a legacy plugin-generated group after removing its label, so
        // narrow fields return to Mautic's native structure.
    ...
    function scheduledControlContainer(input) {
        var container = input && input.parentElement;

        if (hasClass(container, 'mautic-locale-fix-timezone-control')) {
    ...
    function trackTimezoneLabelContainer(input) {
        var container = scheduledControlContainer(input);

        if (runtime.timezoneLabelResizeObserver && typeof runtime.timezoneLabelResizeObserver.observe === 'function') {
            if (container) {
                runtime.timezoneLabelResizeObserver.observe(container);
            }
        }
    }
